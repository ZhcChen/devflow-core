...
|------|----------|----------|
| 需求分析 | 任务已创建 | PRD 草案完成 |
| PRD定稿 | PRD 已落盘且含验收矩阵 | 默认主 Agent 定稿后进入技术设计；触发强制确认条件时需用户确认 |
| 技术设计 | PRD 已定稿 | 技术方案草案完成 |
| 方案定稿 | 技术方案已落盘且满足必备内容 | 默认主 Agent 定稿后进入编码；触发强制确认条件时需用户确认 |
| 编码 | 方案已定稿 | 开发记录与代码变更完成 |
| 审查 | 开发记录已落盘 | 审查通过进入测试 |
| 测试 | 审查通过 | 测试通过进入完成 |
| 完成 | 测试通过 | 任务关闭 |
...
- Orchestrator（主 Agent）：协调流程、管理决策/确认节点、推进阶段流转（见 [orchestrator/](roles/orchestrator/)）
...
- 默认由主 Agent 完成 PRD 与技术方案定稿并继续推进，无需逐步请求用户确认
...
- 允许任务在“方案定稿”后进入待开发清单（见 [backlog/ready.md](../backlog/ready.md)）
- 进入开发前需从清单移除并恢复到“编码”阶段

## 批量预研模式（推荐）

- 一批需求先进入需求池（见 [backlog/intake.md](../backlog/intake.md)）
- 仅执行 PM 与 Architect 阶段，完成 PRD 与 SPEC 的定稿
- 方案定稿后登记待开发清单，主 Agent 提示或用户指示后进入开发
